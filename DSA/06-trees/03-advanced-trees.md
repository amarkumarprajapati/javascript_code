...
function insertAVL(node, val) {
  ...
  if (balance > 1 && val > node.left.val) { node.left = leftRotate(node.left); return rightRotate(node); } // LR
  if (balance < -1 && val < node.right.val) { node.right = rightRotate(node.right); return leftRotate(node); } // RL
  return node;
}
```

## Red-Black Tree
Self-balancing with color properties. Used in `Map`, `Set`, `TreeMap`.

## Practice
- LeetCode: 1382. Balance a Binary Search Tree
- Understand why Map/Search in JS is efficient
